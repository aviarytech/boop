...
export type AuthErrorCode = "UNAUTHORIZED" | "INVALID_TOKEN" | "EXPIRED_TOKEN";
...
export function authErrorData(error: unknown): AuthErrorData | null {
  ...
  if (data.code !== "UNAUTHORIZED" && data.code !== "INVALID_TOKEN" && data.code !== "EXPIRED_TOKEN") return null;
